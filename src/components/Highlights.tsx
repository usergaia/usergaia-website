...
export function S4() {
  ...
  return (
    <>
      <section className="flex h-auto snap-start justify-center bg-yellow-400 text-center text-3xl text-white lg:h-auto">
        <div className="max-w-8xl w-full px-2 sm:px-4">
          <div className="flex flex-col items-center justify-center gap-4 overflow-x-auto pb-4 text-center lg:flex-row lg:gap-0">
            {/* Works Title Section */}
            <div className="mt-4 flex h-64 w-full flex-col items-center justify-center rounded-xl bg-gradient-to-br p-4 sm:h-80 sm:p-6 lg:mr-6 lg:h-98 lg:w-auto">
            ...
            <div className="mt w-full justify-center lg:flex-1">
              ...
              <div className="grid grid-cols-2 gap-3 sm:hidden">
                {pjt.slice(0, 4).map((item, index) => (
                  <div
                    className="flex aspect-square items-center justify-center rounded bg-green-400 text-sm font-semibold text-white"
                    key={index}
                  >
                    {item}
                  </div>
              ...
              <div className="no-scrollbar hidden snap-x space-x-4 overflow-x-auto pb-2 sm:flex lg:max-w-5xl lg:snap-proximity">
                {pjt.map((item, index) => (
                  <div
                    className="flex h-32 w-32 flex-shrink-0 snap-start items-center justify-center rounded bg-red-400 p-94 font-semibold text-white sm:h-48 sm:w-48 lg:mt-6 lg:h-60 lg:w-60 lg:p-50"
                    key={index}
                  >
                    {item}
                  </div>
      ...
      <div className="flex w-full flex-col items-center justify-center bg-green-300 p-8 text-center text-white">
        {/* Heading */}
        <p className="mr-2 mb-4 text-left text-7xl font-bold">Shall we?</p>
        <p className="text-md mb-4 text-left font-thin">
          Lorem ipsum dolor sit amet consectetur, adipisicing elit. Odit
          temporibus in quidem ipsam.
        </p>

        {/* Button */}
        <button className="h-12 w-40 cursor-pointer rounded-full bg-teal-500 text-sm font-semibold text-white transition hover:scale-105 hover:bg-blue-600">
          Email Me
        </button>
      </div>
